...
// Used by GitHub Actions to keep the baseline fresh (weekly).
...
const COMPANIES = [
  ...
  // Biotech (Workday only — Greenhouse/Workable handled separately)
...
function locFromPath(path) {
  ...
  if (slug.includes('---')) return slug.split('---').map(p => p.replace(/-/g, ' ')).join(', ');
  return slug.replace(/--/g, ', ').replace(/-/g, ' ');
}

function normalizeJob(j, c) {
  ...
  const url = path
    ? `https://${c.subdomain}.wd${c.wdNum}.myworkdayjobs.com/en-US/${c.tenant}${path}`
...
(async () => {
  ...
  for (const c of COMPANIES) {
    try {
      let jobs;
      if (c.ats === 'Workday') {
        jobs = await fetchWorkday(c);
      } else if (c.ats === 'Greenhouse') {
        jobs = await fetchGreenhouse(c);
      } else {
        console.log(`  [${c.name}] Skipping (${c.ats} not supported in builder)`);
        continue;
      }
      allJobs.push(...jobs);
    } catch (e) {
